use legion::{system, world::SubWorld, Entity, Query, EntityStore};
...
pub(crate) fn dirty_child(
    world: &mut SubWorld,
    query_transform_with_parent: &mut Query<(Entity, &mut Transform, Option<&Parent>, Option<&Children>)>,
) {
        ...
        while let Some((child, parent)) = tmp_vec.pop() {
            if parents_transform.contains_key(&parent) {
                ...
                if !parent_transform.dirty_child {
                    let mut child_transform_entry =
                        world.entry_mut(child)
                            .expect("An entity has been marked as dirty child but actually does not exist")
                        ;
                    child_transform_entry.get_component_mut::<Transform>()
                        .expect("Missing Transform component previously found on this entity").dirty_child = false;
                    child_transform_entry.get_component_mut::<Transform>()
                    ...
                    parents_transform.insert(child, child_transform_entry.get_component_mut::<Transform>()
                        .expect("Missing Transform component previously found on this entity").clone());
                } else {
                ...
                parents_transform.insert(child, world.entry_mut(child)
                    .expect("An entity has been marked as dirty child but actually does not exist")
                    .get_component_mut::<Transform>()
                    .expect("Missing Transform component previously found on this entity").clone());
            }
    ...
    while first_iter || !transform_entities.is_empty() {
        ...
        while let Some((transform, entities)) = transform_entities.pop() {
            for entity in entities {
                let mut child_entry = world.entry_mut(entity).expect("An entity has been marked as dirty child but actually does not exist");
...
mod tests {
    ...
    use super::*;
    use crate::core::systems::hierarchy_system::*;
    ...
    fn dirty_parent_transform_test() {
        ...
        assert_eq!(1., world.entry(parent).expect("").get_component::<Transform>().unwrap().global_translation.x());
        assert_eq!(2., world.entry(child).expect("").get_component::<Transform>().unwrap().global_translation.x());
        assert_eq!(3., world.entry(child_of_child).expect("").get_component::<Transform>().unwrap().global_translation.x());
        ...
        assert_eq!(6., world.entry(parent).expect("").get_component::<Transform>().unwrap().global_translation.x());
        assert_eq!(7., world.entry(child).expect("").get_component::<Transform>().unwrap().global_translation.x());
        assert_eq!(8., world.entry(child_of_child).expect("").get_component::<Transform>().unwrap().global_translation.x());
    }
}