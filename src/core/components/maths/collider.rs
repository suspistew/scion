...
impl Collider {
    ...
    pub fn new(collider_mask: ColliderMask, collision_filter: Vec<ColliderMask>, collider_type: ColliderType) -> Self {
        Collider{
            collider_mask,
            collider_type,
            collision_filter,
    ...
    pub(crate) fn can_collide_with(&self, other: &Collider) -> bool {
        return self.collision_filter.is_empty() || self.collision_filter.contains(&other.collider_mask)
    }

    pub(crate) fn collides_with(&self, self_transform: &Transform, target_collider: &Collider, target_transform: &Transform) -> bool {
        self.can_collide_with(target_collider) &&
            match (&self.collider_type, &target_collider.collider_type) {
                (ColliderType::Square(self_size), ColliderType::Square(target_size)) => {
                    square_collider_vs_square_collider(SquareColliderInfo::of(self_size, self_transform),
...
fn square_collider_vs_square_collider(self_collider: SquareColliderInfo, target_collider: SquareColliderInfo) -> bool{
    ...
    let (x_min_p1, x_max_p1, y_min_p1, y_max_p1,x_min_p2, x_max_p2, y_min_p2, y_max_p2) = (
        p1.x(), p1.x() + *self_collider.size as f32, p1.y(), p1.y() + *self_collider.size as f32,
        p2.x(), p2.x() + *target_collider.size as f32, p2.y(), p2.y() + *target_collider.size as f32,
        );
    x_min_p1 < x_max_p2 && x_max_p1 > x_min_p2 &&
        y_min_p1 < y_max_p2 && y_max_p1 > y_min_p2
}
...
mod tests {
    ...
    fn test_can_collide_with(){
        ...
        let ship = Collider::new(ColliderMask::Character, vec![ColliderMask::Bullet], ColliderType::Square(5) );
